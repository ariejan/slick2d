...
/**
 * A font implementation that will parse the output of the AngelCode font tool
 * available at:
 * 
 * <a
 ...
 * supports the text format definition file.
 * 
 * @author kevin
 */
public class AngelCodeFont implements Font {
	 ...
	 * 
	 * @param first
	 *            The first character being drawn
	 * @param second
	 *            The second character being drawn
	 * @param value
	 *            The kerning distance between the two characters
	 */
			...
			while (!done) {
				...
				if (line == null) {
					...
					if (line.startsWith("kernings c")) {
						...
						int first = Integer.parseInt(tokens.nextToken()); // first
																			// value
	...
	public int getHeight(String text) {
		...
		for (int i = 0; i < text.length(); i++) {
			...
			maxHeight = Math.max(chars[id].height + chars[id].yoffset,
					maxHeight);
		}
	...
	/**
	 * Cached strings to speed up rendering if we're consistantly rendering the
	...
	private class CachedString {
		 ...
		 * 
		 * @param x
		 *            The x coordinate to render the string at
		 * @param y
		 *            The y coordinate to render the string at
		 * @param start
